...
public class SsqStatsService {
    ...
    public Map<String, Object> predict(int latestN, String strategy, int count, PredictOptions options) {
        ...
        for (int i = 0; i < c; i++) {
            picks.add(pickOne(latestN, s, i, options));
        }
    ...
    private Pick pickFromDraws(List<SsqDraw> draws, String strategy, int salt, PredictOptions options) {
        ...
        if ("markov".equalsIgnoreCase(s) || "ml".equalsIgnoreCase(s)) {
            redTrans = buildRedTransition(ordered);
            blueTransFromLast = buildBlueFromLastTransition(ordered);
        }
        if ("bayes".equalsIgnoreCase(s) || "ml".equalsIgnoreCase(s)) {
    ...
    private Pick pickRaw(int[] redCnt, int[] blueCnt, int[] redMiss, int[] blueMiss,
        ...
        if ("ml".equalsIgnoreCase(s)) {
            ...
            int[] mlRedScore = buildMlRedScore(redCnt, redMiss, markovRed, ordered);
            int[] mlBlueScore = buildMlBlueScore(blueCnt, blueMiss, blueTransFromLast, ordered);

            List<Integer> red = weightedSampleWithoutReplacementInRange(mlRedScore, redMiss, 6, 1, 33, rnd, opt);
            int blue = weightedSampleOneWithFilter(mlBlueScore, blueMiss, 1, 16, rnd, opt);
            return new Pick(red, blue);
        }
