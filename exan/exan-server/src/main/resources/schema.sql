...
CREATE TABLE IF NOT EXISTS `paper` (
  ...
  KEY `idx_paper_stage_subject_date` (`stage_id`, `subject_id`, `paper_date`),
  UNIQUE KEY `uk_paper_stage_subject_name_date` (`stage_id`, `subject_id`, `name`(191), `paper_date`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS `paper_content` (
  ...
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_paper_content_paper_id` (`paper_id`),
  KEY `idx_paper_content_created` (`created_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS `paper_stat` (
  `id` BIGINT NOT NULL AUTO_INCREMENT,
  `paper_id` BIGINT NOT NULL,
  ...
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_paper_stat_paper_id` (`paper_id`),
  KEY `idx_paper_stat_updated` (`updated_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
