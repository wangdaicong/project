...
const inflightKlines = new Map();
async function ensureKlinesBestEffort(code, limit) {
    ...
    if (after.length >= need)
        return { rows: after, source: 'fresh' };
...
function computeThemeBoost(sector, newsKeywords) {
    ...
    for (const rule of THEME_RULES) {
        ...
        boost += rule.weight;
        if (links.length < 5)
            links.push(`${hitKw} → ${rule.theme}（匹配：${hitSec}，+${rule.weight}）`);
    }
    return { boost, links };
}
function makeThemeFirstRecoFromQuote(code, quote, newsKeywords) {
    ...
    if (theme.links.length)
        triggers.push(`主题链路：${theme.links.join('；')}`);
    ...
    try {
        ...
        if (cached.length < Math.min(60, q.data.limit)) {
            try {
                ...
                if (fresh.length) {
                    store.upsertKlines(idx.symbol, fresh.map((k) => ({
                        code: idx.symbol,
                        ts: k.ts,
                        open: k.open,
                        high: k.high,
...
apiRouter.get('/reco', async (req, res) => {
    ...
    const rows = cached.map((x) => ({ ts: x.ts, close: x.close }));
    ...
    if (!reco) {
        try {
            ...
            if (qt?.price != null)
                reco = makeFallbackRecoFromPrice(q.data.code, qt.price, newsKeywords);
        }
        catch {
            // ignore
        }
...
apiRouter.get('/reco/top', async (req, res) => {
    ...
    async function buildTop() {
        ...
        for (let i = 0; i < candidateCodes.length; i += concurrency) {
            ...
            const results = await withTimeout(Promise.all(batch.map(async (code) => {
                const cached = mapStoreQuote(store.getQuote(code));
                ...
                if (cached && cached.price != null) {
                    if (!cached.sector && remaining > 600) {
                        try {
                            ...
                            return { code, quote: qt ?? cached, from: 'provider' };
                        }
                        catch {
                            return { code, quote: cached, from: 'store' };
                        }
                    }
                    return { code, quote: cached, from: 'store' };
                }
                try {
                    const qt = await withTimeout(provider.getQuote(code), Math.min(quoteTimeoutMs, remaining));
                    return { code, quote: qt, from: 'provider' };
                }
                catch {
                    return { code, quote: null, from: 'timeout' };
                }
            })), Math.min(2500, Math.max(600, remaining))).catch(() => []);
            for (const r of results) {
                if (r.quote)
                    quotes.set(r.code, r.quote);
                fetched++;
            }
        }
        const scored = [];
        for (const code of candidateCodes) {
            ...
            const k = (k0 || []).map((x) => ({ ts: Number(x.ts), close: Number(x.close) })).filter((x) => Number.isFinite(x.ts) && Number.isFinite(x.close));
        ...
        const out = scored.slice(0, want).map(({ score: _s, ...rest }) => rest);
        if (out.length < want) {
...
apiRouter.get('/reco/theme', async (req, res) => {
    const q = z
        .object({
    ...
    async function build() {
        ...
        for (let i = 0; i < candidateCodes.length; i += concurrency) {
            ...
            })), Math.min(2800, Math.max(600, remaining))).catch(() => []);
            for (const r of results) {
                if (r.quote)
                    quotes.set(r.code, r.quote);
                fetched++;
        ...
        for (const code of candidateCodes) {
            ...
            const k0 = store.getKlines(code, 160);
            const k = (k0 || [])
                .map((x) => ({ ts: Number(x.ts), close: Number(x.close) }))
...
apiRouter.post('/reco/keyword', async (req, res) => {
    const q = z
        .object({
