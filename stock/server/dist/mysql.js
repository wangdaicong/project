...
let pool = null;
function decryptAes256Gcm(enc, keyB64) {
    const key = Buffer.from(keyB64, 'base64');
    if (key.length !== 32)
        throw new Error('bad_key');
    const parts = String(enc || '').split(':');
    if (parts.length !== 3)
        throw new Error('bad_enc');
...
function resolveMysqlPassword() {
    ...
    if (!pwd)
        throw new Error('MYSQL_PASSWORD_NOT_SET');
    return pwd;
}
export function getMysqlConfigFromEnv() {
    const user = process.env.MYSQL_USER;
    if (!user)
        throw new Error('MYSQL_USER_NOT_SET');
    return {
...
export async function ensureMysqlTables() {
  ...
  `);
    // 兼容：表已存在但缺少注释时补齐
    try {
        await p.execute("ALTER TABLE reco_top_cache " +
    ...
    try {
        const [cols] = await p.query("SELECT DATA_TYPE as t FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='reco_top_cache' AND COLUMN_NAME='ts' LIMIT 1");
...
export async function mysqlHealthy() {
    try {
        const p = await getMysqlPool();
