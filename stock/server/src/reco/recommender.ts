...
export type Reco = {
  ...
  score: number;
  summary: string;
  signals: Array<{ key: string; label: string; value: string; bias: 'bullish' | 'bearish' | 'neutral' }>;
  triggers: string[];
  risks: string[];
  newsKeywords: string[];
  reason: string;
...
export function computeRecoScore(reco: Pick<Reco, 'signals' | 'newsKeywords'>): number {
  let s = 0;
  for (const sig of reco.signals || []) {
    if (sig.bias === 'bullish') s += 4;
    else if (sig.bias === 'bearish') s -= 4;
    else s += 0;

    if (sig.key === 'macd_cross' && sig.value.includes('可能')) s += 4;
    if (sig.key === 'macd_hist' && sig.value.includes('0轴上方')) s += 2;
    if (sig.key === 'trend_5d' && sig.value.includes('偏强')) s += 2;
  }
...
export function makeReco(code: string, k: { ts: number; close: number }[], newsKeywords: string[] = []): Reco | null {
  if (k.length < 35) return null;
  ...
  const macdBias: 'bullish' | 'bearish' | 'neutral' = lastMacd.macd > 0 ? 'bullish' : lastMacd.macd < 0 ? 'bearish' : 'neutral';
  const trendBias: 'bullish' | 'bearish' | 'neutral' = trendUp ? 'bullish' : 'bearish';
  ...
  if (crossUp) reasonParts.push('信号：MACD出现由弱转强的“翻红”迹象（仅供参考）');
  if (newsKeywords.length) reasonParts.push(`时事面：近期高频主题 ${newsKeywords.slice(0, 5).join(' / ')}（仅供参考）`);
