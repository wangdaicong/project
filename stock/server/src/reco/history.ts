...
export type RecoRunType = 'top' | 'keyword';

export type RecoRunInsert = {
  runType: RecoRunType;
...
export async function ensureRecoHistoryTables(): Promise<void> {
  ...
  try {
    const [cols] = await p.query<any[]>(
      "SELECT DATA_TYPE as t FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='reco_run' AND COLUMN_NAME='created_at' LIMIT 1"
    );
  ...
  try {
    const [cols] = await p.query<any[]>(
      "SELECT DATA_TYPE as t FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='reco_item' AND COLUMN_NAME='quote_ts' LIMIT 1"
    );
    const t = String(cols?.[0]?.t ?? '').toLowerCase();
    if (t === 'bigint') {
  ...
  try {
    const [cols] = await p.query<any[]>(
      "SELECT DATA_TYPE as t FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='reco_item' AND COLUMN_NAME='created_at' LIMIT 1"
    );
    const t = String(cols?.[0]?.t ?? '').toLowerCase();
    if (t === 'bigint') {
...
export async function insertRecoRunWithItems(run: RecoRunInsert, items: RecoItemInsert[]): Promise<number | null> {
  try {
    ...
    const [r]: any = await p.execute(
      'INSERT INTO reco_run (run_type, query_text, keywords_json, limit_n, took_ms, created_at) VALUES (?, ?, ?, ?, ?, ?)',
      [run.runType, run.queryText, keywordsJson, run.limit, run.tookMs, createdAt]
    );
    ...
    if (items.length) {
      ...
      await p.query(
        'INSERT INTO reco_item (run_id, rank_n, code, name, market, sector, quote_ts, price, pct, score, entry, exit_price, risk, summary, reason, signals_json, triggers_json, risks_json, news_keywords_json, created_at) VALUES ?',
        [values]
      );
    }
