...
export async function createStockTables(): Promise<void> {
  ...
  try {
    const [cols] = await p.query<any[]>(
      "SELECT DATA_TYPE as t FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='quotes' AND COLUMN_NAME='ts' LIMIT 1"
    );
  ...
  try {
    const [cols] = await p.query<any[]>(
      "SELECT DATA_TYPE as t FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='quotes' AND COLUMN_NAME='inserted_at' LIMIT 1"
    );
    const t = String(cols?.[0]?.t ?? '').toLowerCase();
    if (t === 'bigint') {
      ...
      await p.execute(
        "ALTER TABLE quotes CHANGE COLUMN inserted_at_dt inserted_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '入表时间'"
      );
    } else {
      await p.execute(
        "ALTER TABLE quotes MODIFY COLUMN inserted_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '入表时间'"
      );
    }
  } catch {
    // ignore
  ...
  try {
    const [cols] = await p.query<any[]>(
      "SELECT DATA_TYPE as t FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='klines' AND COLUMN_NAME='ts' LIMIT 1"
    );
    const t = String(cols?.[0]?.t ?? '').toLowerCase();
    if (t === 'bigint') {
  ...
  try {
    const [cols] = await p.query<any[]>(
      "SELECT DATA_TYPE as t FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='news' AND COLUMN_NAME='ts' LIMIT 1"
    );
    const t = String(cols?.[0]?.t ?? '').toLowerCase();
    if (t === 'bigint') {
  ...
  try {
    const [cols] = await p.query<any[]>(
      "SELECT DATA_TYPE as t FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='news' AND COLUMN_NAME='inserted_at' LIMIT 1"
    );
    const t = String(cols?.[0]?.t ?? '').toLowerCase();
    if (t === 'bigint') {
      ...
      await p.execute(
        "ALTER TABLE news CHANGE COLUMN inserted_at_dt inserted_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '入表时间'"
      );
    } else {
      await p.execute(
        "ALTER TABLE news MODIFY COLUMN inserted_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '入表时间'"
      );
    }
  } catch {
    // ignore
  ...
  try {
    await p.query(
      "CREATE EVENT IF NOT EXISTS ev_purge_quotes_1m " +
  ...
  try {
    await p.query(
      "CREATE EVENT IF NOT EXISTS ev_purge_news_1m " +
        "ON SCHEDULE EVERY 1 MONTH STARTS (CURRENT_TIMESTAMP + INTERVAL 1 MONTH) " +
...
export async function fetchAllQuotes(limitRows = 6000): Promise<QuoteRow[]> {
  const p = await getMysqlPool();
  const lmt = Math.max(1, Math.min(50_000, Math.floor(limitRows || 6000)));
  const [rows] = await p.query<any[]>('SELECT * FROM quotes ORDER BY ts DESC LIMIT ?', [lmt]);
  return (rows ?? []).map((r) => ({
    code: String(r.code),
...
export async function fetchSymbolsByIndexTag(indexTag: string): Promise<SymbolRow[]> {
  const p = await getMysqlPool();
  const [rows] = await p.query<any[]>('SELECT code,name,market,index_tag FROM symbols WHERE index_tag=? ORDER BY code', [indexTag]);
  return (rows ?? []).map((r) => ({ code: String(r.code), name: String(r.name), market: String(r.market), index_tag: String(r.index_tag) }));
}

export async function fetchAllSymbols(): Promise<SymbolRow[]> {
  const p = await getMysqlPool();
  const [rows] = await p.query<any[]>('SELECT code,name,market,index_tag FROM symbols');
  return (rows ?? []).map((r) => ({ code: String(r.code), name: String(r.name), market: String(r.market), index_tag: String(r.index_tag) }));
}

export async function countSymbolsByIndexTag(indexTag: string): Promise<number> {
  const p = await getMysqlPool();
  const [rows] = await p.query<any[]>('SELECT COUNT(1) as c FROM symbols WHERE index_tag=?', [indexTag]);
  return Number(rows?.[0]?.c ?? 0);
}

export async function upsertSymbols(indexTag: string, rows: Array<{ code: string; name: string; market: string }>): Promise<void> {
  if (!rows.length) return;
  const p = await getMysqlPool();
  const now = new Date();
  const values = rows.map((r) => [indexTag, r.code, r.name, r.market, now]);
  await p.query(
    'INSERT INTO symbols (index_tag, code, name, market, inserted_at) VALUES ? ON DUPLICATE KEY UPDATE name=VALUES(name), market=VALUES(market), inserted_at=VALUES(inserted_at)',
    [values]
  );
}

export async function fetchQuote(code: string): Promise<QuoteRow | null> {
  const p = await getMysqlPool();
  const [rows] = await p.query<any[]>('SELECT * FROM quotes WHERE code=? LIMIT 1', [code]);
  const r = rows?.[0];
  if (!r) return null;
  return {
    code: String(r.code),
...
export async function replaceQuotes(rows: QuoteRow[]): Promise<void> {
  if (!rows.length) return;
  const p = await getMysqlPool();
  ...
  await p.query(
    'INSERT INTO quotes (code,name_cn,ts,price,open,high,low,prev_close,volume,amount,pct,sector,inserted_at) VALUES ? ON DUPLICATE KEY UPDATE name_cn=VALUES(name_cn), ts=VALUES(ts), price=VALUES(price), open=VALUES(open), high=VALUES(high), low=VALUES(low), prev_close=VALUES(prev_close), volume=VALUES(volume), amount=VALUES(amount), pct=VALUES(pct), sector=VALUES(sector), inserted_at=VALUES(inserted_at)',
    [values]
  );
}

export async function upsertKlines(code: string, rows: KlineRow[]): Promise<void> {
  if (!rows.length) return;
  const p = await getMysqlPool();
  const values = rows.map((k) => [k.code, new Date(k.ts), k.open, k.high, k.low, k.close, k.volume, k.amount]);
  await p.query(
    'INSERT INTO klines (code,ts,open,high,low,close,volume,amount) VALUES ? ON DUPLICATE KEY UPDATE open=VALUES(open), high=VALUES(high), low=VALUES(low), close=VALUES(close), volume=VALUES(volume), amount=VALUES(amount)',
    [values]
  );
}

export async function fetchKlinesByCode(code: string, limit: number): Promise<KlineRow[]> {
  const p = await getMysqlPool();
  const lmt = Math.max(1, Math.min(1000, Math.floor(limit || 200)));
  const [rows] = await p.query<any[]>(
    'SELECT code,ts,open,high,low,close,volume,amount FROM klines WHERE code=? ORDER BY ts DESC LIMIT ?',
    [code, lmt]
  );
...
export async function fetchRecentKlines(limitRows: number): Promise<KlineRow[]> {
  const p = await getMysqlPool();
  const lmt = Math.max(1, Math.min(200_000, Math.floor(limitRows || 50_000)));
  const [rows] = await p.query<any[]>(
    'SELECT code,ts,open,high,low,close,volume,amount FROM klines ORDER BY ts DESC LIMIT ?',
    [lmt]
  );
  return (rows ?? []).map((r) => ({
    code: String(r.code),
...
export async function fetchNews(limit: number): Promise<NewsItem[]> {
  ...
  const [rows] = await p.query<any[]>('SELECT id,ts,title,url,source,summary,inserted_at FROM news ORDER BY ts DESC LIMIT ?', [lmt]);
  return (rows ?? []).map((r) => ({
    id: String(r.id),
    ts: (r.ts instanceof Date ? r.ts.getTime() : new Date(r.ts).getTime()),
...
export async function replaceNews(items: NewsItem[]): Promise<number> {
  ...
  await p.query(
    'INSERT INTO news (id,ts,title,url,source,summary,inserted_at) VALUES ? ON DUPLICATE KEY UPDATE ts=VALUES(ts), title=VALUES(title), url=VALUES(url), source=VALUES(source), summary=VALUES(summary), inserted_at=VALUES(inserted_at)',
    [values]
  );
  return items.length;
}
